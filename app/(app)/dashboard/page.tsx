...
//     description: "Upload or capture crop images to detect plant diseases using AI.",
//     href: "/ai-model",
//    imageUrl: "/images/cropdisease.png",
...
const featureCards = [
  {
    ...
    description: "Upload or capture crop images to detect plant diseases using AI.",
    href: "/ai-model",
    imageUrl: "/images/cropdisease.png", // ⭐ YOUR IMAGE HERE
  },
  {
    title: "Livestock Care",
    description: "Monitor and manage animal health records.",
    ...
    <div className="relative min-h-full w-full overflow-hidden">
      ...
      {bgImage && (
        <Image
          ...
          alt="Farm background"
          fill
          className="z-0 opacity-20 blur-sm object-cover"
        />
      )}
      <div className="absolute inset-0 bg-gradient-to-b from-black/30 via-transparent to-black/30 z-10" />
      ...
      <div className="relative z-20 p-4 sm:p-6 lg:p-8">
        ...
        >
          {featureCards.map((card) => {
            // ⭐ If imageUrl exists (custom image) use it
            let imageSrc = card.imageUrl;

            // ⭐ Otherwise use placeholderImages
            if (!imageSrc) {
              const cardImage = placeholderImages.find(
                (p) => p.id === card.imageId
              );
              imageSrc = cardImage?.imageUrl;
            }
