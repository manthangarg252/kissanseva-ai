...
import { Button } from "@/components/ui/button";
import {
  recommendApplicableSchemes,
  type RecommendApplicableSchemesOutput,
} from "@/ai/flows/recommend-applicable-schemes";
...
export default function SchemesPage() {
  const [recommendedSchemes, setRecommendedSchemes] =
    useState<RecommendApplicableSchemesOutput | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
  ...
  const isRecommended = (schemeName: string) =>
    recommendedSchemes?.some((rs) => rs.name === schemeName);

  const getRecommendationReason = (schemeName: string) =>
    recommendedSchemes?.find((rs) => rs.name === schemeName)?.reason;
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>Personalized AI Recommendations</CardTitle>
          <CardDescription>
            Click to analyze your profile and find the most suitable schemes.
          </CardDescription>
        </CardHeader>
        ...
        {governmentSchemes.map((scheme: Scheme) => (
          <Card
            ...
            className={`flex flex-col transition-all ${
              isRecommended(scheme.name) ? "border-primary shadow-lg" : ""
            }`}
          >
            <CardHeader>
              <CardTitle>{scheme.name}</CardTitle>
              <Badge
                variant={
                  isRecommended(scheme.name) ? "default" : "secondary"
                }
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-primary/80">
                        {getRecommendationReason(scheme.name)}
                      </p>
                    </div>
                  </div>
                </div>
